...
feature "user updates an event" do
  ...
  let(:attributes) { { name: Faker::Name.title, description: Faker::Lorem.paragraph, amount: Faker::Number.decimal(4).to_f, day: 13 } }

  background do
    login_user(user)
  end

  scenario 'update event from free to paid as creator and free event' do
    visit edit_event_path(event)

    update_event_from_free_to_paid_with(attributes)

    expect(page.find('form')).to have_css('.event_budget_amount')
